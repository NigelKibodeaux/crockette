...
app.whenReady()
    .then(() => {
        ...
        if (hasDomainAndPass) mainWindow.loadFile('index.html')
    ...
    .then(async () => {
        try {
            // Check for updates to Crockette
            ...
            dialog.showMessageBoxSync({
                type: 'warning',
                message: 'Could not check for updates to Crockette',
            })
        }
    })
...
ipcMain.on('auth-updated', (event, { domain, password }) => {
...
ipcMain.on('get-download-directory', (event, key) => {
    const downloadDirectory = settings.get('downloadDirectory')
    mainWindow.webContents.send('download-directory-set', downloadDirectory)
})

ipcMain.handle('setSetting', (event, key, value) => {
    log('setSetting:', { event, key, value })
...
ipcMain.on('show-logs', () => mainWindow.webContents.toggleDevTools())
